...
def HW_func(S,N):
    E=S[0,1:] # E is the sum of the vectors
    NE=np.float64(N[0,1:]) # NE is the number of not-nan antries for each coordinate of the vectors
    Mean=E/NE # Mean is the Mean vector (ignoring nans)
    O=S[1:,1:] # O is the sum of the outer products
    NO=np.float64(N[1:,1:]) # NO is the number of non-nans in the outer product.
